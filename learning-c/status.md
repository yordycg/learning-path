...
- S3 D4 (Jue 3): **`execvp` + PATH** — code-first: intentar `4-systems/01-syscalls-processes/7-execvp-path.c` sin leer. Contexto en `MOC - Processes`: hoy en `6-exec-wait.c` pasaste `"ls"` a `execvp` SIN ruta completa — la `p` hizo el PATH lookup por ti. Hoy se profundiza en cómo la shell busca el binario en `$PATH` (vs ruta absoluta/relativa) y por qué `execvp` y no `system()`. Conexión: ¿qué pasaría si escribes `execvp("mycmd", ...)` y `mycmd` no está en ningún directorio del `PATH`? Recurso JIT: [exec/execvp](https://www.youtube.com/watch?v=-_7BcxBr4P4) (Sorber) · `man 3 execvp` / `man 7 environ`. **Pendientes de D3 (si queda tiempo al abrir D4):** ejercicios `01` (estado de salida con `WIFEXITED`/`WEXITSTATUS`), `02` (cazar un zombie en `Z` con `ps` mientras el padre `sleep` sin `wait`), `03` (proto-mysh: bucle `fgets` → array runtime → `fork`+`execvp`+`wait`). Zettel D3 `Linux - exec and Zombie Reaping.md` **aún NO generado** (cierre con tiempo justo) — generarlo en D4.

...
- S3 D3 (Mié 2): **`exec` family + `wait`** — code-first: intentar `4-systems/01-syscalls-processes/6-exec-wait.c` sin leer, fallar con propósito, luego recurso JIT ([Sorber exec/execvp](https://www.youtube.com/watch?v=-_7BcxBr4P4), `man 2 execve`/`wait`). Contexto en `MOC - Processes` (Obsidian): `fork` ya visto (crea al hijo), hoy se reemplaza la imagen del proceso (`exec`) y el padre recoge al hijo (`wait`), zombies y reaping. Conexión: hoy el padre de `5-fork-basic.c` NO esperaba al hijo — ¿qué le pasaba a ese hijo si el padre salía primero? Ejercicios: básico (fork + exec + wait de `ls`) → aplicado (contar zombies con `ps` mientras el padre espera) → integrado (ciclo fork+execvp+wait de mysh). Zettel al cierre: `Linux - exec and Zombie Reaping.md`.

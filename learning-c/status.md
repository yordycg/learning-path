...
- **S3 Sáb (Sáb 5): MILESTONE — mysh v1.0** — `projects/mysh/src/mysh.c` (v1.0). Comandos externos con `fork` + `execvp` + `wait`, **sin** `system()`. Ya tienes las piezas sueltas de D1–D5: fork/exec/wait/errno/WEXITSTATUS. Hoy se integran: bucle `fgets` → parsear argv → `fork`+`execvp`+`wait` → propagar `$?`. Referencias: Brennan (parte v1.0) + UCI 143A. `git tag v1.0`. **Pendientes arrastrados que puedes meter si el milestone lo pide:** rama `WIFSIGNALED`/`WTERMSIG` (muerte por señal) aún sin implementar en la propagación de estado; exercises de D3 (`zombie` en `Z` con `ps` y proto-mysh) quedaron como refuerzo opcional.

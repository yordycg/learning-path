...
- **S4 Mié (Mié 9): D3 — Disposiciones en fork/exec** (corazón de mysh v1.5): herencia por fork, exec resetea handlers capturados→default pero **mantiene SIG_IGN** → hijo debe resetear a `SIG_DFL` antes de `exec`. Crear `3-expert/07-signals/3-fork-exec-disposition.c` con el Contrato diario.

...
- **S4 Jue (Jue 10): D4 — `SIGCHLD` + reaping async**: instalar handler + `waitpid(WNOHANG)`. Crear `3-expert/07-signals/4-sigchld-reap.c` con el Contrato diario.
- **Pendiente de D3 (retomar antes del milestone Sáb 12 mysh v1.5):** demostrar la 2ª mitad de la asimetría — que `SIG_IGN` **sobrevive** al exec (a diferencia del handler capturado que se resetea). Es el gotcha que obliga al hijo de mysh a resetear SIGINT a `SIG_DFL` entre `fork` y `execvp`. (exercise `03-*.c`)
